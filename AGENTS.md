...
- Last rewritten: 2026-09-06 (late evening), after the first passing `sspm-local` Shadow run plus benchmark audit metadata, the single-system summary schema fix, context-selection affinity scoring, Develop context caps, explicit benchmark changed-file scope enforcement, and missing-pytest classification in Tests MCP.
...
- A local attempt to rerun `sspm-local` after the allowed-scope change could not exercise the provider because this shell has no `OPENAI_API_KEY`; it exited quickly with zero tokens and no patch, and the throwaway result file was removed.
...
- **The `sspm-local` fixture has produced one passing Shadow-only run but no paired PASS/FAIL gate yet.** `2026-09-06-sspm-scale-shadow-pytest-unavailable.json` is enough to validate Shadow's internal lifecycle for the task, but the report still cannot calculate reduction or retention until baseline and Shadow are run together.
...
- **The `sspm-local` real-repository fixture now passes Shadow-only and isolates the next savings target.** The latest live run selected only `core/config.py`, changed only `core/config.py`, passed `timeout-present` plus `scope-limited`, skipped Test due unavailable pytest/test command, and completed Validate. Total tokens fell from 14,584 in the allowed-scope failure to 4,911 because unnecessary remediation was eliminated. Remaining waste is mostly the fixed Plan frontier call and Develop's verbose patch generation.
...
Run the `sspm-local` fixture as a paired comparison from a shell with `OPENAI_API_KEY` set:

```bash
node packages/shadow/dist/cli/index.js benchmark run fixtures/benchmarks/sspm-local \
  --benchmark-id sspm-scale \
  --task config-default-timeout \
  --work-root "$WORK_ROOT" \
  --out docs/benchmarks/results/2026-09-06-sspm-scale-paired-pytest-unavailable.json \
  --report
```

Expected shape after the latest code: Shadow criteria 1/1, completed, zero irrelevant files, zero retries, and roughly 4,900 total tokens. The gate should now be judged against the single-frontier baseline; the saved Shadow-only run is not enough to report reduction or retention.

If the paired report passes cleanly, the next savings work is to route Plan off frontier for low-complexity edits or make Plan deterministic until complexity crosses a threshold. In the latest passing run Plan spent 1,004 frontier tokens before Develop touched one file; that is now the clearest remaining bottleneck for this task.
