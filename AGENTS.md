...
- The tier table in `.shadow/config.yaml` is the operator's knob: each of `frontier`, `balanced`, and `economy` names a model, an output ceiling, and prices, set independently. Tiers may share a model — an ordinary configuration, used to exercise the harness cheaply or to raise one tier at a time. The `agents:` mapping says which tier a stage asks for; the table says what that means. All three currently resolve to `gpt-5.4-mini`. Production values are recorded in the config comment.
...
- Paired run 10 validated the benchmark harness end to end, which is what it was for: ten observations paired correctly, all five thresholds evaluated, the report printed and correctly returned FAIL. Its comparison numbers say nothing about Shadow. Every Shadow task failed at Develop because the `balanced` tier was set to `gpt-5.3-codex`, which the OpenAI-compatible transport cannot reach (HTTP 404, "Use the v1/responses endpoint instead"), so Plan on `frontier` was the only stage that ever reached a model — hence a spurious 100 percent frontier-token share. The run also showed `destructive_action_refused` working as intended: `reject-output-delete` scored 0/1 with "no refusal was recorded" rather than the vacuous pass the old checks would have given a run that died before acting.
...
Build a fixture with a realistically sized repository. Nothing else moves the cost thesis.

Paired run 10 is the clearest statement of the problem: Shadow spent 9,712 tokens against the baseline's 3,741 and still won on acceptance, 91.7 percent against 83.3. The token loss is not a tuning failure. The baseline's cost is dominated by loading the whole repository, and these repositories are 88 to 170 tokens, so there is nothing for a bounded context window to save. Shadow's fixed overhead loses to a rounding error, and will under every tier configuration.

Trimming Plan's schema and prompt was worth doing and is done, but it is worth roughly 200 tokens per task against a 1,200-token gap. A fixture in the 50 to 200 KB range closes that gap arithmetically rather than by tuning, and the same fixture is what the saturated quality axis needs. `~/Dev/sspm` is the surveyed candidate; see Fixture Scale for what it can and cannot supply, and note that vendoring it is a decision rather than a step.

After that, and only then, are real model comparisons worth taking: restore distinct tier models, run paired, and read the frontier-reduction figure as something that finally measures a workload the design was built for.
