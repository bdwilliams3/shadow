...
- Last rewritten: 2026-09-07 (early morning), after the single-model baseline gained one bounded repair attempt with artifact evidence, aggregate retry budgeting, retry accounting in benchmark observations, compiled support for loose `*** Update File` apply-patch fragments after malformed diff preambles, order-independent placement for unique apply-patch hunks, the same task contract Shadow receives, pre-apply repair for valid patches that exceed relevant-file scope, and corrected benchmark accounting for Shadow calls to the selected baseline provider/model. The CLI package was rebuilt. A live paired run could not be completed from the Codex shell because `OPENAI_API_KEY`, `ANTHROPIC_API_KEY`, and `GEMINI_API_KEY` were not present in that environment.
...
Improve the first-run and configuration experience around direct model aliases. The core product should let a developer say "Plan uses this model, Design uses that model, Develop uses another model" without learning stale tier vocabulary, then verify those choices before the first expensive run.

Immediate next step: rerun the paired `sspm-local` benchmark once from a terminal where the provider API-key environment variables are loaded, using `--benchmark-model gpt-5.6-luna`, and confirm the report prints nonzero Shadow `Reserved/baseline-model tokens` when Plan uses the same resolved provider/model as the baseline plus `Model usage` cost lines for both systems. After that, return to the product-critical onboarding slice: make `shadow init`, `config validate`, the readiness check, and provider probes work as a coherent setup path for provider-grouped direct model aliases, including placeholder/fictional model detection and clearer reporting before any expensive run.
