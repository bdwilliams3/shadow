...
- Last rewritten: 2026-09-06 (late evening), after the first passing paired `sspm-local` Shadow-vs-baseline benchmark plus benchmark audit metadata, the single-system summary schema fix, context-selection affinity scoring, Develop context caps, explicit benchmark changed-file scope enforcement, and missing-pytest classification in Tests MCP.
...
- The Plan stage still cannot justify a frontier call for low-complexity edits. In the paired `sspm-local` pass, Plan consumed 956 frontier tokens before Develop touched one file. That is acceptable relative to the baseline's 27,971-token whole-repository call, but it is now Shadow's clearest remaining waste on this task.
...
Route Plan off `frontier` for low-complexity edits, or make Plan deterministic until complexity crosses a threshold. The paired `sspm-local` benchmark now passes, so the old gate is closed. In that passing run Plan spent 956 frontier tokens before Develop touched one file; Shadow still beat the baseline decisively, but this fixed Plan call is now the clearest remaining bottleneck for the task.

The implementation should preserve the current positive routing signal: `config-default-timeout` should continue to run Plan -> Develop -> Test -> Validate, skip Design, change only `core/config.py`, and pass the paired `sspm-local` report. Add or update unit coverage around the complexity threshold and tier choice, then rerun the paired fixture to measure frontier-token reduction after the change.
