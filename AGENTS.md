...
- Last rewritten: 2026-09-08, after the command-line chatbox product slice added a bare `shadow` startup preflight, configured stage/model visibility, compact interactive progress from durable run events, local parsing for slash or bare commands, one-token input guardrails, and a configurable read-only chat coordinator at `agents.chat`. The coordinator returns `keep_thinking`, `change_directions`, or `start_coding`; discussion inputs route through it instead of the lifecycle. A first version incorrectly treated `models/`, bare `exit`, and conversational questions as lifecycle requests; those defects are fixed and covered by tests. The CLI package was rebuilt.
