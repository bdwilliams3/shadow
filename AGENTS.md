...
- Last rewritten: 2026-09-07 23:03 EDT. The command-line chatbox slice is verified with bare `shadow` startup preflight, configured stage/model visibility, compact interactive progress from durable run events, local slash/bare command parsing, one-token input guardrails, and a configurable read-only chat coordinator at `agents.chat`. The coordinator returns `keep_thinking`, `change_directions`, or `start_coding`; discussion inputs route through it instead of the lifecycle. The repository OpenAI catalog now includes `gpt-5-3-codex` resolving to `gpt-5.3-codex` with the requested 6,000-token ceiling and `$0.20`/`$1.20` accounting rates. Config validation, both TypeScript checks, the 33-file/168-test suite, and `git diff --check` pass. The CLI package was rebuilt.
...
- `shadow` opens a readline-based interactive terminal session scoped to the current working directory. Startup now prints a concise deterministic preflight before any model-spending request: workspace path, Git branch/dirty-file count, provider key readiness by environment variable name only when missing, Tests MCP health, the main model-backed stage aliases, run budget, and config sources. The full lifecycle stage/provider/model table is intentionally behind `models` or `/models`.
...
- Bare `shadow` is now the primary command-line chatbox path for the first product slice. Conversational/read-only text is routed to `ChatOrchestratorAgent`, using deterministic local repo context plus the configured `agents.chat` model alias; action-shaped requests still start the lifecycle through the existing orchestrator, print the created run id, and stream progress from durable events. Full records still live under `.shadow`.
...
- Interactive commands include `help`, `status [run-id]`, `plan [run-id]`, `budget [run-id]`, `diff [run-id]`, `approvals`, `approve [approval-id]`, `reject [approval-id]`, `cancel`, `models`, `actions`, `preflight`, `btw`, `steer`, and `exit`; a leading slash is optional, and a trailing slash on a command word such as `models/` is tolerated. `btw` and `steer` append steering notes to the coordinator/lifecycle prompt. Unknown single-token inputs such as `fix` or `src/` are treated as incomplete instead of launching a model-backed run. Multi-word questions such as `hello what are your capabilities?` and `what can we do here?` stay in the chat coordinator and do not create a lifecycle run. The loop uses readline's async iterator so piped command sequences terminate cleanly.
...
#### Mechanical, 2026-09-08

- After the first chatbox UX slice and corrective command/coordinator pass: `./node_modules/.bin/tsc --noEmit -p packages/shadow/tsconfig.json` passes; `./node_modules/.bin/tsc -p packages/shadow/tsconfig.build.json` passes; `./node_modules/.bin/vitest run packages/shadow/test/cli-format.test.ts packages/shadow/test/cli-interactive.test.ts packages/shadow/test/chat-orchestrator-agent.test.ts packages/shadow/test/config.test.ts --testTimeout 10000` passes 15/15. The coordinator model is configured as `agents.chat`; there is no separate top-level `chat` configuration section.
- After fixing OpenAI strict schema compatibility for the chat coordinator: `./node_modules/.bin/vitest run packages/shadow/test/openai-compatible.test.ts packages/shadow/test/chat-orchestrator-agent.test.ts --testTimeout 10000` passes 9/9. The compiled chat schema now requires `decision`, `reply`, `proposedObjective`, and `confidence`, with `proposedObjective` typed as `["string", "null"]`.
- Compiled CLI smoke tests pass: `printf 'models/\nexit\n' | node packages/shadow/dist/cli/index.js`, `printf 'fix\nexit\n' | node packages/shadow/dist/cli/index.js`, and `printf 'help\nexit\n' | node packages/shadow/dist/cli/index.js` all exit 0. `models/` prints the model table, bare `exit` exits, and `fix` reports that no run was started because a one-token input is incomplete.
- Broader regression after the chatbox slice: `./node_modules/.bin/vitest run packages/shadow/test --testTimeout 10000` passes 33 files and 168 tests; `git diff --check` passes.
...
- The interactive terminal has a configurable chat coordinator for read-only discussion, but it is still a first slice rather than the final terminal product. `ChatOrchestratorAgent` can decide `start_coding`, but the CLI does not yet keep an editable proposed objective and approval loop before starting the lifecycle. Active runs are awaited by the same loop, so `/cancel` cannot be typed while a model or tool call is in flight; same-session interruption needs either keypress handling, a background run task, or both. First-run setup guidance also needs refinement: missing provider keys are visible in preflight, but the chatbox does not yet explain the shortest path to a working local configuration.
...
Continue the command-line chatbox product slice. The next high-leverage step is the coordinator approval loop: keep a proposed objective in chat, let `btw` / `steer` revise it, and start the lifecycle only after the user approves the coordinator's `start_coding` proposal.

Immediate next step: extend `ChatOrchestratorAgent` and the interactive runner in `packages/shadow/src/cli/index.ts` so `keep_thinking`, `change_directions`, and `start_coding` drive an explicit conversational state machine. The coordinator should continue shaping the objective until it can show a concise plan/proposed lifecycle and ask for approval. After that, add same-session cancellation/interruption for active runs, then run the paired benchmark as regression evidence.
